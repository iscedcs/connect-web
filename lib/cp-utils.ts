...
export function formatAuditAction(
  ...
  const event = (log.eventType || log.event || log.action || '').toUpperCase().trim()
  const target =
    log.targetResource ||
    log.targetName ||
    log.resourceName ||
  ...
  switch (event) {
    ...
    case 'ORGANIZATION_CREATED':
      return target ? `Organization '${target}' created` : 'Organization created'
    ...
    case 'STAFF_JOINED':
    case 'MEMBER_ADDED':
      return target ? `New team member added: ${target}` : 'New team member added'
    ...
    case 'APPOINTMENT_CANCELLED':
      return target ? `Appointment cancelled: ${target}` : 'Appointment cancelled'
    case 'APPOINTMENT_RESCHEDULED':
      return target ? `Appointment rescheduled: ${target}` : 'Appointment rescheduled'
    ...
    case 'UPDATE':
      if (entity) {
        const cleanEntity = entity.replace(/^Cp/, '').replace(/([A-Z])/g, ' $1').trim()
    ...
    default: {
      if (!event) return target || actor ? `Activity by ${actor}` : 'Recent activity'
      const clean = event
        .toLowerCase()
        .replace(/_/g, ' ')
