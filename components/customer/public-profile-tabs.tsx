...
export default function PublicProfileTabs({
  ...
  const socialItems = connectItems.filter((item) => {
    ...
    return (
      platform && platform !== "youtube" && SOCIAL_PLATFORMS.includes(platform)
    );
  });
                ...
                {socialItems.map((item) => (
                  <button
                    ...
                    <div className="w-14 h-14 rounded-full bg-black/60 flex items-center justify-center">
                      <img
                        src={item.icon || getFaviconFromUrl(item.url, 64)}
                        alt={item.title}
                  ...
                  if (!isVideo) {
                    return (
                      <a
                        ...
                        aria-label={item.title || "Open YouTube channel"}
                    ...
                    <div className="flex items-start gap-4 flex-1">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-[10px] text-white/50 truncate max-w-[180px]">
                          {item.username || resolvePublicUrl(item.url, id)}
                        </p>
                      </div>
                    </div>
