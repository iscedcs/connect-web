    ...
    <div className="space-y-6">
        ...
        <div>
          ...
          <p className="text-xs text-white/70">
            ...
            {artisan.averageRating > 0
              ? `${Number(artisan.averageRating).toFixed(1)} avg`
              : "No ratings yet"}
          </p>
        </div>
      </div>
      ...
      {artisan.averageRating > 0 && (
        <div className="bg-gradient-to-br from-purple-600/30 to-purple-900/30 rounded-2xl p-5 border border-purple-500/20 flex items-center gap-4">
          ...
          <div className="text-xs text-white/80 space-y-1">
            <p>{artisan.totalReviews} total reviews</p>
            <p>
      ...
      {reviews.length === 0 ? (
        <div className="bg-white/5 rounded-xl p-8 text-center">
          <Star className="size-8 mx-auto mb-3 text-white/40" />
          <p className="text-sm font-medium mb-1 text-white">No reviews yet</p>
          <p className="text-xs text-white/60">
            Reviews will appear here after customers rate your services.
          </p>
        </div>
      ) : (
