...
interface ArtisanCardProps {
  ...
  compact?: boolean;
  /** Pass the connect profile ID so the setup link is only shown when a profile exists */
...
function BecomeArtisanCTA({
  ...
  const canSetup = !!profileId;
  return (
    <div
      className={`bg-neutral-900 rounded-3xl flex flex-col ${compact ? "p-5 gap-4" : "p-8 gap-8"}`}
    >
      ...
      <div>
        ...
        <div className="flex items-start justify-between mt-3">
          ...
          {canSetup ? (
            ...
            <ArrowRight className="size-5 mt-1 text-white/20" />
          )}
        </div>
      </div>
    </div>
          ...
          {isActive && (
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="bg-white/5 rounded-xl p-3 text-center">
                ...
                <p className="text-[10px] text-white/50 mt-1">Reviews</p>
              </div>
              <div className="bg-white/5 rounded-xl p-3 text-center">
                ...
                <p className="text-[10px] text-white/50 mt-1">Bookings</p>
              </div>
            </div>
          )}
...
export default function ArtisanCard({
  ...
  if (!artisanProfile) {
    return <BecomeArtisanCTA compact={compact} profileId={profileId} />;
  }
