...
function headlineFeatures(limits: PlanLimits): { label: string; on: boolean }[] {
	return [
		{ label: `${formatLimit(limits.profiles)} profile${limits.profiles === 1 ? '' : 's'}`, on: true },
		{ label: `${formatLimit(limits.socialLinks)} social links`, on: limits.socialLinks !== 0 },
		{ label: `${formatLimit(limits.videos)} videos`, on: limits.videos !== 0 },
		{ label: `${formatLimit(limits.files)} files · ${formatStorage(limits.storageBytes)}`, on: limits.files !== 0 },
		{ label: `${formatLimit(limits.customForms)} custom forms`, on: limits.customForms !== 0 },
		{
		...
		{ label: 'Listed in artisan directory', on: limits.artisanDirectoryEligible },
		{ label: 'Verified badge', on: limits.verifiedBadge },
		{ label: 'Priority support', on: limits.prioritySupport },
	];
}

export default function SubscriptionClient({ subscription, plans, limits }: Props) {
	...
	async function post(path: string, body?: unknown) {
		...
		}))) as { success: boolean; message: string; authorizationUrl?: string };
	}
	...
	async function handleChoose(plan: Plan) {
		...
		try {
			...
			const result = hasSubscription
				? await post('/api/subscriptions/initiate-payment', { planKey: plan.key })
				...
				{subscription ? (
					...
					<div className='rounded-xl border border-white/10 bg-white/[0.03] p-4'>
						...
						<p className='text-xs text-white/40 mt-1'>
							Choose a plan below to get started. Paid plans open with a
							3-month trial — no charge today.
						</p>
					</div>
				)}
			</section>
