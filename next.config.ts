...
function wsDomain(url?: string) {
  if (!url) return "";
  try {
    return new URL(url).origin.replace(/^https:\/\//, "wss://");
  } catch {
    return "";
  }
}
...
const API_DOMAINS = API_URLS.map(domain).filter(Boolean).join(" ");
const WS_DOMAINS = API_URLS.map(wsDomain).filter(Boolean).join(" ");
    ...
    return [
      {
        ...
        headers: [
          ...
          // Content Security Policy - prevent XSS and injection attacks
          {
            ...
            value: [
              "default-src 'self'",
              "script-src 'self' 'unsafe-eval' 'unsafe-inline' 'wasm-unsafe-eval' https://maps.googleapis.com https://*.googleapis.com https://www.gstatic.com",
              "worker-src 'self' blob:",
              "style-src 'self' 'unsafe-inline' https://fonts.googleapis.com",
              "font-src 'self' https://fonts.gstatic.com data:",
              "img-src 'self' data: https://cdn.sanity.io https://*.digitaloceanspaces.com https://*.s3.amazonaws.com https://*.amazonaws.com https://i.ytimg.com https://*.blob.vercel-storage.com https://*.gstatic.com https://www.google.com https://api.dicebear.com blob:",
